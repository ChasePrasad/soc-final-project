/*
 * image_filter.c — Hardware/Software image filter for the Urbana Board
 *
 * Loads a PNG. For modes 0-3, it converts it to the 0x00RRGGBB format expected
 * by the AXI4-Stream filter IP, transfers it through the AXI DMA, and saves the
 * result. For mode 4, it applies a software blur directly in C.
 *
 * Usage:
 * ./image_filter <input.png> <filter_mode> <output.png>
 *
 * Filter modes:
 * 0 — Passthrough (Hardware)
 * 1 — Invert (Hardware)
 * 2 — Grayscale (Hardware)
 * 3 — Neon Duotone (Hardware)
 * 4 — Box Blur (Software)
 *
 * Build:
...
// --- NEW FUNCTION: Software Blur ---
void apply_software_blur(unsigned char *img, int width, int height) {
    // Allocate a temporary buffer to hold the original pixels
    unsigned char *temp = malloc(width * height * 3);
    if (!temp) return;
    memcpy(temp, img, width * height * 3);

    // Simple 3x3 Box Blur
    for (int y = 1; y < height - 1; y++) {
        for (int x = 1; x < width - 1; x++) {
            for (int c = 0; c < 3; c++) { // For Red, Green, and Blue channels
                int sum = 0;
                // Accumulate the 9 pixels around (and including) the center pixel
                for (int dy = -1; dy <= 1; dy++) {
                    for (int dx = -1; dx <= 1; dx++) {
                        sum += temp[((y + dy) * width + (x + dx)) * 3 + c];
                    }
                }
                // Average them out
                img[(y * width + x) * 3 + c] = sum / 9;
            }
        }
    }
    free(temp);
...
int main(int argc, char *argv[])
{
    ...
    // UPDATED: Now allows mode 4
    if (filter_mode < 0 || filter_mode > 4) {
        fprintf(stderr, "Error: filter_mode must be 0, 1, 2, 3, or 4\n");
        return 1;
    }
    ...
    // --- NEW LOGIC: Check for Software Blur (Mode 4) ---
    if (filter_mode == 4) {
        printf("Applying Software Box Blur...\n");
        apply_software_blur(rgb, width, height);
        ...
        stbi_image_free(rgb);
        printf("Done.\n");
        return 0; // Exit early, no need for DMA/Hardware
    }
    // ---------------------------------------------------
    ...
    printf("Setting hardware filter mode: %d\n", filter_mode);
